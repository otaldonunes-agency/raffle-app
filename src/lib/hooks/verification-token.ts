...
export function useCheckVerificationToken<TError = DefaultError>(args: { operation: PolicyCrudKind; where?: { id?: string; identifier?: string; token?: string }; }, options?: (Omit<UseQueryOptions<boolean, TError, boolean>, 'queryKey'> & ExtraQueryOptions)) {
    const { endpoint, fetch } = getHooksContext();
